...
import { hashHostId } from '@/components/AdminControls';
...
export default function HostAudienceView({ params }: { params: Promise<{ hostId: string; audience: string }> }) {
  const { hostId, audience: audienceParam } = use(params);
  ...
  const renderContent = () => {
    switch (currentSlide) {
      ...
      case SlideType.SEASONS: return <SeasonSlide data={hostData} />;
